// BudgetFlow OCR 배치 테스트 — 멀티모델 벤치마킹 (수정판)
// 한국 영수증 20개 (HumynLabs) + 인도네시아 영수증 80개 (CORD) = 100개
//
// 수정 사항 (2026-06-20): 기존 버전은 Sonnet 호출 시 Express 서버(/analyze/image)를
// 거쳤는데, 서버가 시작 시점에 고정한 모델을 그대로 썼던 버그가 있었음
// (서버는 계속 Haiku로 떠 있었으므로 "Sonnet" 결과가 실제로는 Haiku였을 가능성).
// → Haiku/Sonnet 모두 Anthropic API를 모델 파라미터와 함께 직접 호출하도록 수정.
// → 한국/CORD 분리 상호명 정확도 통계도 추가.
//
// 실행:
//   TEST_MODEL=haiku  npx tsx src/test-ocr-batch.ts
//   TEST_MODEL=sonnet npx tsx src/test-ocr-batch.ts
//   TEST_MODEL=gemini npx tsx src/test-ocr-batch.ts
...
const TEST_MODEL = (process.env.TEST_MODEL ?? "haiku") as "haiku" | "sonnet" | "gemini";
const CLAUDE_MODEL_MAP: Record<string, string> = {
  haiku:  "claude-haiku-4-5-20251001",
  sonnet: "claude-sonnet-4-5-20250929",
};
const MODEL_LABEL = TEST_MODEL;
...
// 실제 프로덕션 프롬프트 파일을 그대로 읽어서 사용 (일관성 유지)
...
// bedrockClient.ts의 OCR_TOOL과 동일한 스키마
...
async function callClaudeVisionDirect(model: string, base64: string): Promise<any> {
...
function buildSummary(rows: any[]) {
  ...
  return {
    ...
    "상호명 추출 성공률 (한국)":pct(korOk.filter(r=>r.상호명성공==="Y").length, korOk.length),
  ...
  for (let i = 0; i < DATASETS.length; i++) {
    ...
    try {
      ...
      if (TEST_MODEL === "gemini") {
        const raw = await callGeminiVision(buildOcrPromptText(CATEGORIES_TEXT), base64, "image/jpeg");
        result = normalizeResult(raw);
      } else {
        const raw = await callClaudeVisionDirect(CLAUDE_MODEL_MAP[TEST_MODEL], base64);
        result = normalizeResult(raw);
      }
