...
export function buildAccountantPacket(
  ...
  if (highFindings.length === 0) {
    ...
    for (const f of highFindings) {
      lines.push(
        `- [${f.expenseId}] ${f.merchant ?? "상호 미확인"} / ${f.amount != null ? f.amount.toLocaleString() + "원" : "-"} / ${f.findingType}: ${f.reviewReason}`,
      );
    }
  }
  ...
  for (const e of ready) {
    lines.push(
      `| ${e.date} | ${e.merchant} | ${e.amount != null ? e.amount.toLocaleString() + "원" : "-"} | ${e.tax_invoice_type} | ${e.payment_method} | ${e.vat_class} |`,
    );
  }

  lines.push(
    "",
    "> 이 자료는 세무 신고 준비용 참고 자료입니다. 최종 신고는 세무사를 통해 진행하세요.",
  );
  return lines.join("\n");
}
...
export function buildSelfFilingPacket(
  ...
  return {
    ...
    summary: {
      ...
      vatCandidates: ready.filter((e) => e.vat_class === "vat_credit_candidate")
        .length,
    },
    feeImpact,
    items: ready.map((e) => ({
      ...
      merchant: e.merchant,
      amount: e.amount,
      taxInvoiceType: e.tax_invoice_type,
      paymentMethod: e.payment_method,
      vatClass: e.vat_class,
      vatReason: e.vat_reason,
      businessPurpose: e.business_purpose,
      deductibility: e.deductibility,
    })),
    notice:
      "이 자료는 세무 신고 준비용 참고 자료입니다. 최종 신고는 세무사 또는 홈택스를 통해 진행하세요.",
  };
}
...
const evidenceLabelKo: Record<string, string> = {
  card_receipt: "카드영수증",
  cash_receipt: "현금영수증",
  tax_invoice: "세금계산서",
...
const paymentMethodLabelKo: Record<string, string> = {
  corporate_card: "법인카드",
  personal_card: "개인카드",
  cash: "현금",
  transfer: "계좌이체",
...
// pg가 DATE 컬럼을 JS Date로 파싱해 넘기는 경우(타입 선언은 string)가 있어 우선 처리.
...
export function buildSelfFilingCsv(expenses: TaxExpenseRow[]): string {
  ...
  const rows = ready.map((e) => {
    ...
    return [
      ...
      vat,
      total,
      paymentMethodLabelKo[e.payment_method ?? "unknown"] ?? "미확인",
      evidenceLabelKo[e.tax_invoice_type ?? "unknown"] ?? "미확인",
