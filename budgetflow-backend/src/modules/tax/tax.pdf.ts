...
export function buildAccountantPacketPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    const review = expenses.filter(
      (e) => e.tax_review_status === "needs_review",
    );
    ...
    doc
      .fillColor(SUB)
      .fontSize(11)
      .text("BudgetFlow · 세무사 전달 자료", left, doc.y, { width });
    doc
      .fillColor(INK)
      .fontSize(22)
      .text("세무 신고 준비 패킷", left, doc.y, { width });
    doc.fillColor(SUB).fontSize(10);
    doc.text(`프로젝트: ${projectName}`, left, doc.y, { width });
    doc.text(`신고 기간: ${period}`, left, doc.y, { width });
    doc.text(`생성일: ${formatDateKo(new Date().toISOString())}`, left, doc.y, {
      width,
    });
    hr();
    ...
    if (high.length === 0) {
      ...
      for (const f of high) {
        ...
        doc
          ...
          .fontSize(10)
          .text(
            `• ${f.merchant ?? "상호 미확인"} / ${won(f.amount)}`,
            left,
            doc.y,
            { width },
          );
        doc
          .fillColor(RED)
          .fontSize(9)
          .text(`   ${f.reviewReason}`, left, doc.y, { width });
        doc.fontSize(10);
      }
    }
    ...
    if (ready.length === 0) {
      doc
        .fillColor(SUB)
        .fontSize(9)
        .text("준비 완료된 지출이 없습니다.", left, doc.y, { width });
    } else {
